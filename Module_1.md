...
>⚠️ Важно: После редактирования файла /etc/sudoers рекомендуется выполнить проверку синтаксиса командой visudo -c. Для применения изменений перезагрузка системы не требуется.
